...
CONFIGURA
    ...
    clrf DECENAS			; El display empieza en 0.

; PROCESO PARA RECIBIR DATOS Y MANDAR LLAMAR LA OPERACION ADECUADA.
NUMERO1
    movff DECENAS, LATA			; Antes, encender el resultado anterior.
    call RECORRIDO
...
OPERACIONES
    ...
    goto CONVERSION			; Convertir a decenas y unidades antes de la proxima repeticion.
...
CONVERSION
    ...
    movff NUM1, DECENAS
CUENTA_DECENAS
    movff DECENAS, TEMP2		; Antes de restarle, guardar el resultado anterior en TEMP2.
    subwf DECENAS, F, A			; Se resta DECENAS-10 y se guarda en NUM1.
    movf DECENAS, F, A			; Activar DECENAS en STATUS.
    btfsc STATUS, N, A
...
CUENTA_DECENAS_FINAL
    ...
    rlcf DECENAS, F, A			; Rotar 4 veces para que quede en la posicion mas significativa.
    rlcf DECENAS, F, A
    rlcf DECENAS, F, A
    rlcf DECENAS, F, A
    movf TEMP2, W, A			; Mover el valor almacenado en TEMP2 (residuo) a WREG.
    addwf DECENAS, F, A			; Sumar decenas con residuo y almacenarlo en DECENAS.
    goto NUMERO1
